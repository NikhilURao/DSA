...
class EditDistance 
{
	...
	public static int EditDistanceTopDown(String s, String t) {
		
		int l = s.length(); int m = t.length();
		
		return EditDistanceRec(s, t, l-1, m-1);
		
	}
	
	public static int EditDistanceRec(String s, String t, int l, int m) {
		
		if(l==0) return m;
		if (m==0) return l;
		
		if (s.charAt(l) == t.charAt(m)) return EditDistanceRec(s,t,l-1,m-1);
		
		else {
			int insert, delete, replace;
			insert = EditDistanceRec(s,t,l,m-1);
			delete = EditDistanceRec(s,t,l-1,m);
			replace = EditDistanceRec(s,t,l-1,m-1);
  ...
  public static void main(String args[]) {
    ...
    System.out.println(EditDistanceTopDown(s,t));
  }
